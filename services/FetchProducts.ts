...
export async function fetchCategories(): Promise<CategoriesResponse> {
  try {
    const data = await apiFetch<CategoriesResponse>(
      API_CONFIG.ENDPOINTS.karyawan.categories.list,
    );
    if (!data.success) {
...
export async function fetchProductsSearch(
  ...
  try {
    ...
    const data = await apiFetch<ProductsSearchResponse>(
      API_CONFIG.ENDPOINTS.karyawan.products.search(branchName, page, limit),
    );
    if (!data.success) {
    ...
    throw error instanceof Error
      ? error
      : new Error("Failed to fetch products search");
  }
}

export async function fetchProductsPopular(
  branchName: string,
  limit: number = 100,
): Promise<ProductsPopularResponse> {
  try {
    const data = await apiFetch<ProductsPopularResponse>(
      API_CONFIG.ENDPOINTS.karyawan.products.popular(branchName, limit),
    );
    if (!data.success) {
    ...
    throw error instanceof Error
      ? error
      : new Error("Failed to fetch products popular");
  }
}
