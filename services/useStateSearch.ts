...
export function useStateSearch(): UseStateSearchResult {
  ...
  const handleOpenScanner = useCallback(async () => {
    ...
    try {
      ...
      const Camera =
        (cameraModule as any).Camera ?? (cameraModule as any).default;
      const CameraView =
      ...
      result = result.filter((p) => {
        ...
        const barcode =
          typeof rawBarcode === "string"
            ? rawBarcode.toLowerCase()
            : String(rawBarcode).toLowerCase();
        const category =
  ...
  const handleBarCodeScanned = useCallback(
    ({ data: scannedData }: { data: string }) => {
      ...
      const found = products.find((p) => {
        const rawBarcode = p.barcode ?? "";
        const barcodeStr =
          typeof rawBarcode === "string" ? rawBarcode : String(rawBarcode);
