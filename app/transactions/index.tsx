...
import { PAYMENT_STATUS_OPTIONS, STATUS_OPTIONS } from "@/assets/data/Data"
    ...
    if (isLoading && records.length === 0) {
        return (
            <View className="flex-1 bg-gray-50 items-center justify-center">
                ...
                <Text className="mt-3 text-gray-500">Memuat transaksi...</Text>
            </View>
        )
    }
        ...
        <View className="flex-1 bg-white">
            <View className="bg-white border-b border-gray-100 px-4 pt-2 pb-4">
                <View className="flex-row items-center">
                    ...
                    <Text className="flex-1 ml-3 text-lg font-bold text-gray-900">Semua Transaksi</Text>
                </View>
                <View className="mt-4 flex-row gap-3">
                    ...
                    >
                        ...
                        <Text className="text-sm font-medium text-gray-700 ml-1.5">Filter</Text>
            ...
            >
                <View className="pb-6">
                    <Text className="text-xs font-bold text-gray-400 tracking-widest px-1 pt-2 pb-2">
                        STATUS TRANSAKSI
                    </Text>
                    {STATUS_OPTIONS.map((opt) => (
                        <TouchableOpacity
                            ...
                            className={`py-4 border-b border-gray-100 flex-row items-center justify-between ${filterStatus === opt.value ? 'bg-gray-50' : ''}`}
                        >
                            <View className="flex-row items-center">
                                <View className="w-9 h-9 rounded-xl bg-gray-100 items-center justify-center mr-3">
                                    <Ionicons
                                        name={opt.icon}
                                        size={20}
                                        color={filterStatus === opt.value ? '#7C3AED' : '#6B7280'}
                                    />
                                </View>
                                <Text
                                    className={`font-medium ${filterStatus === opt.value ? 'text-gray-900' : 'text-gray-600'}`}
                                >
                                    {opt.label}
                                </Text>
                            </View>
                            {filterStatus === opt.value && <Ionicons name="checkmark" size={20} color="#7C3AED" />}
                        </TouchableOpacity>
                    ))}
                    <Text className="text-xs font-bold text-gray-400 tracking-widest px-1 pt-4 pb-2">
                        STATUS PEMBAYARAN
                    </Text>
                    {PAYMENT_STATUS_OPTIONS.map((opt) => (
                        <TouchableOpacity
                            ...
                            className={`py-4 border-b border-gray-100 flex-row items-center justify-between ${filterPaymentStatus === opt.value ? 'bg-gray-50' : ''}`}
                        >
                            <View className="flex-row items-center">
                                <View className="w-9 h-9 rounded-xl bg-gray-100 items-center justify-center mr-3">
                                    <Ionicons
                                        name={opt.icon}
                                        size={20}
                                        color={filterPaymentStatus === opt.value ? '#7C3AED' : '#6B7280'}
                                    />
                                </View>
                                <Text
                                    className={`font-medium ${filterPaymentStatus === opt.value ? 'text-gray-900' : 'text-gray-600'}`}
                                >
                                    {opt.label}
                                </Text>
                            </View>
                    ...
                    <View className="flex-row">
                        ...
                        <View className="flex-1 px-5 py-5">
                            <Text className="text-xs tracking-widest text-gray-400 font-bold">
                                RINGKASAN {filterStatus !== 'all' || filterPaymentStatus !== 'all' || filterMonth !== 'all' || searchQuery.trim() ? '(FILTER)' : ''}
                            </Text>
                ...
                {filtered.length === 0 ? (
                    <View className="bg-white rounded-2xl border border-gray-100 p-8 items-center">
                        <View className="w-16 h-16 rounded-full bg-gray-100 items-center justify-center mb-4">
                            <Ionicons
                                name={searchQuery.trim() || filterStatus !== 'all' || filterPaymentStatus !== 'all' || filterMonth !== 'all' ? 'filter-outline' : 'receipt-outline'}
                                size={32}
                                color="#9CA3AF"
                            />
                        </View>
                        <Text className="text-gray-900 font-semibold text-center">
                            {searchQuery.trim() || filterStatus !== 'all' || filterPaymentStatus !== 'all' || filterMonth !== 'all'
                                ? 'Tidak ada transaksi yang sesuai'
                                : 'Belum ada transaksi'}
                        </Text>
                        <Text className="text-gray-500 text-sm text-center mt-1">
                            {searchQuery.trim() || filterStatus !== 'all' || filterPaymentStatus !== 'all' || filterMonth !== 'all'
                                ? 'Ubah pencarian atau reset filter'
